<?php 
  ...
  if(isset($_SESSION['name'])){
    ...
    if(isset($_POST['search'])){
      ...
      $whrSQL = "WHERE (image_caption LIKE '%".$searchKeyword."%' OR image_textfile LIKE '%".$searchKeyword."%')";
      }
    }

    $result = $con->query("SELECT * FROM images $whrSQL ORDER BY image_id DESC");
        ...
        <?php
          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              $image_caption = !empty($searchKeyword)?highlightWords($row['image_caption'],    $searchKeyword):$row['image_caption'];
              $image_textfile = !empty($searchKeyword)?highlightWords($row['image_textfile'],  $searchKeyword):$row['image_textfile'];
