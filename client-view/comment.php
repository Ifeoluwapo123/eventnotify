  ...
  <section id="mu-menu">
    <div class="container-fluid">
      <div class="row content">
        <div class="col-sm-3 sidenav">
          ...
          <ul class="nav nav-pills nav-stacked">
            ...
            <li><?php echo "<a href='config/log-out.php'><strong><h2 style ='color:black; text-align: left; font-size:15px;'>Log-out</h2></strong></a></tr>"?></li> 
          </ul><br>     
          <form class="mu-search-form" action="search.php?jkfj%hmbf%jisbf=getInfo" method="POST">
      ...
      <div class="col-sm-9 blog">
        ...
        <hr>
         <?php echo "<a href='config/log-out.php'><strong><h2 style ='color:#ffffff; background-color:#00BFFF; width:20%; text-align: center; font-size:20px; font-family:sans-serrif; border-radius:20px; padding: 5px; float:right'>Log out</h2></strong></a></tr>"?>
         <br><br>
         <hr>
         <?php
          $d = $_GET['imageid'];
          $query = "SELECT image_id, image_textfile,image_caption,image_date FROM images WHERE image_filename LIKE '%".$d."'";
          $result = mysqli_query($con, $query) or die(mysqli_error($con)); 
          $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
          foreach ($rows as $row) {
            echo "<h4><strong style='font-family:sans-serrif; font-size: 25px;'>Event Title/Description:</strong> ".$row['image_caption']."</h4>";
            echo "<p><strong style='color:red'>Posted on:</strong>"." ".$row['image_date']."</p>";
            ?>
            <a href="pictures/<?php echo $d;?>"><img src='../pictures/<?php echo $d;?>' class='thumbnail'></a>
            <p><?php echo $row['image_textfile'] ?></p>
            <hr>
            <div id="com">
              <?php     
              $count = "SELECT image_filename FROM comments WHERE image_filename ='".$d."'";
              $query = mysqli_query($con, $count) or die(mysqli_error($con));  
              $number = mysqli_num_rows($query);?>
              <li><a href=<?php echo "comment.php?imageid=".$d?>>Comment<?php echo " "."<b>".$number."</b>"?></a></li>
              <li class="sharethis-inline-share-buttons"></li>
              <li class='li'><a href=<?php echo "like2.php?imageid=".$d?>>Like</a>
                  <em <?php if(userLiked($row['image_id'])){?>
                  class ="fa fa-thumbs-up like-btn"
              <?php }else{ ?>
                  class ="fa fa-thumbs-o-up like-btn" 
              <?php 
                  } 
              ?>data-id="<?php echo $row['image_id'];?>"></em>
                  <span class ='likes'><b><?php echo getLikes($row['image_id'])?></b></span>
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              </li>
            </div>       
          <?php
        ...
        $_SESSION['d'] = $d;
        $query = 'CREATE TABLE IF NOT EXISTS comments (comment_id  INTEGER  NOT NULL AUTO_INCREMENT, image_username VARCHAR(255) NOT NULL, image_filename VARCHAR(255) NOT NULL DEFAULT "", comment_file TEXT NOT NULL,  PRIMARY KEY (comment_id)) ENGINE=MyISAM';
        mysqli_query($con, $query) or die (mysqli_error($con));
        ?>
        <hr>
        <form action="config/post_comment.php?post%/by%comment" method="POST">
          <textarea name ="comment" cols="40" rows="3" placeholder="Enter a Comment"></textarea>
          <input type="submit" name="subCom" value="Comment"> 
        </form>
        <?php    
        $query = "SELECT image_username, comment_file FROM comments WHERE image_filename ='".$_SESSION['d']."' LIMIT 10";
        $result = mysqli_query($con, $query) or die(mysqli_error($con));
        while($row = mysqli_fetch_array($result)){
          echo "<div style =''>";
          extract($row);
          $que = "SELECT image_filename FROM profileimages WHERE username ='".$image_username."'";
          //set_error_handler('my_error_handler1');
          $re = mysqli_query($con, $que) or die(mysqli_error($con));
          $post = mysqli_fetch_array($re);
          extract($post);?>
          <a href='Profilepictures/<?php echo $image_filename;?>'><img src='../Profilepictures/<?php echo $image_filename;?>' class='thumbnail' style = 'width: 10%; height:45px; margin-bottom: 3px'></a>
        <?php
          echo "<strong>".$image_username."</strong>";
          echo "<br/>";
          echo $comment_file;
          echo "</div>";
          echo "<hr>";
